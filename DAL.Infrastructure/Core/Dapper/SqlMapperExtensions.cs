...
namespace Dapper.Contrib.Extensions
{
    public static class SqlMapperExtensions
    {
        ...
        /// <param name="id">Id of the entity to get, must be marked with [Key] attribute</param>
        /// <returns>Entity of T</returns>
        public static ICollection<T> GetAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            string sql;
            if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
           ...
           ICollection<T> obj = null;

            //if (type.IsInterface)
            //{
            //    var res = connection.Query(sql).ToList() as IDictionary<string, object>;
            ...
            //else
            //{
                obj = connection.Query<T>(sql, transaction: transaction, commandTimeout: commandTimeout).ToList();
            //}
            return obj;
        }
